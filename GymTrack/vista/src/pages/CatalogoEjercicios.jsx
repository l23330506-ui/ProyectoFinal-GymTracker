...
export default function CatalogoEjercicios() {
  ...
  useEffect(() => {
    // Conexión directa a la API en el puerto 5050
    fetch('http://localhost:5050/api/ejercicios') // Ajusta el endpoint final si mapearon la ruta de otra forma en C#
      .then((response) => {
        if (!response.ok) {
          throw new Error('Error al recuperar el catálogo de ejercicios');
        }
  ...
  if (cargando) {
    return <div style={{ textAlign: 'center', padding: '40px', fontSize: '1.2rem', color: '#666' }}>⏳ Extrayendo catálogo desde la API...</div>;
  }
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      <div style={{ marginBottom: '30px' }}>
        ...
        <p style={{ margin: '0 0 20px 0', color: '#666' }}>Sincronizado en tiempo real con la API del servidor en el puerto 5050.</p>
      ...
      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fee2e2', color: '#dc2626', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
           <strong>Error de comunicación:</strong> No se pudo conectar a la base de datos a través de http://localhost:5050. Verifica que el Backend esté encendido.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
        {ejerciciosFiltrados.length > 0 ? (
          ...
          <p style={{ gridColumn: '1 / -1', textAlign: 'center', color: '#999', padding: '40px' }}>
            {error ? "No hay datos disponibles por error de red." : "No se encontraron ejercicios que coincidan con la búsqueda."}
          </p>
        )}
      </div>
    </div>
